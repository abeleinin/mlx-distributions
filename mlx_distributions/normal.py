...
class Normal:
    ...
    def sample(self, sample_shape):
        return mx.random.normal(shape=sample_shape, dtype=self.loc.dtype, loc=self.loc, scale=self.scale)

    def log_prob(self, value: mx.array) -> mx.array:
        variance = self.scale ** 2
        return -0.5 * (mx.log(2 * mx.pi * variance) + ((value - self.loc) ** 2) / variance)
